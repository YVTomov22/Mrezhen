# 🧠 Mrezhen: Connect. Conquer. Thrive.

## 🌟 Project Overview

**Mrezhen** is a unique, supportive social media platform designed to connect individuals facing similar life challenges. It moves beyond passive communication by integrating an **AI companion** that actively guides users through their personal struggles, transforming difficulties into achievable goals.
...
To run Mrezhen on your **local machine**, you need to have the following core technologies installed:
...
Follow these steps to set up and run the Mrezhen project locally.
...
2.  **Create Database:** Use the `psql` command-line tool or a GUI client (like pgAdmin) to create the project database.
    ...
    ```
3.  **Configuration:** Update the database connection settings in the appropriate configuration file (e.g., a `.env` file) to match your local PostgreSQL credentials (username, password, and database name `mrezhen_db`).
...
Once all services are running, you should be able to access the Mrezhen application in your web browser, typically at:
