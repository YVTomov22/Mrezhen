...
export function MilestoneItem({ milestone }: MilestoneProps) {
  ...
  return (
    <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden transition-all">
      {/* Milestone Header */}
      <div 
        className="p-6 border-b bg-muted/50 flex flex-col md:flex-row justify-between md:items-center gap-4 cursor-pointer hover:bg-muted transition-colors"
        onClick={() => setIsExpanded(!isExpanded)}
      >
        <div className="flex items-start gap-3">
            {/* Toggle Icon */}
            <div className={cn("mt-1 p-1 rounded-md bg-accent text-muted-foreground transition-transform", isExpanded && "rotate-180")}>
      ...
      {isExpanded && (
        <div className="p-6 bg-muted/30 animate-in fade-in slide-in-from-top-2 duration-200">
          {milestone.quests.length === 0 ? (
            <div className="text-center py-8 border border-dashed rounded-lg bg-card/50">
              <p className="text-sm text-muted-foreground italic mb-2">{t("noQuestsYet")}</p>
